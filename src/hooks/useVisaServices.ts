...
export function useVisaServices() {
  ...
  const addVisaService = async (newService: VisaServiceFormData) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to create visa service",
      );
      throw err;
    }
  };
  ...
  const updateVisaService = async (
    id: string,
    updatedService: Partial<VisaServiceFormData>,
  ) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to update visa service",
      );
      throw err;
    }
  };
  ...
  const deleteVisaService = async (id: string) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to delete visa service",
      );
      throw err;
    }
  };
